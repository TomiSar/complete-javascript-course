...
const account1 = {
  ...
  movementsDates: [
    ...
    '2024-02-03T17:01:17.194Z',
    '2024-02-08T20:36:17.929Z',
...
const account3 = {
  owner: 'Pekka Sauri',
  movements: [35000, 1455.23, -706.5, 25000, -781.29, -133.9, 479.97, 150000],
  interestRate: 1.6, // %
  pin: 3333,
...
const formatMovementDate = function (date, locale) {
  ...
  const calulateDaysPassed = (date1, date2) =>
    Math.round(Math.abs(date2 - date1) / dayToMs);

  const daysPassed = calulateDaysPassed(new Date(), date);
...
const startLogOutTimer = function () {
  const tick = function () {
    ...
    // When 0 seconds remaining log out user
    if (time === 0) {
      clearInterval(timer);
...
btnLogin.addEventListener('click', function (event) {
  ...
  if (currentAccount?.pin === +inputLoginPin.value) {
    ...
    // const locale = navigator.language; locales from browser
    labelDate.textContent = new Intl.DateTimeFormat(
      currentAccount.locale,
      dateOptions
    ).format(now);

    // Timer clear timer if exists
    if (timer) clearInterval(timer);
    timer = startLogOutTimer();

    // Update UI
    updateUI(currentAccount);
  } else if (!containerApp.style.opacity) {
    // !labelWelcome.textContent.includes("Welcome back to Bankist";
    // If containerApp is not visible, it means no user is logged in
    labelWelcome.textContent += '(Wrong user and/or PIN)';
  }
...
const showTextOnElement = function (element) {
  setTimeout(() => {
...
// console.log(+'23'); //string to int

// // Parsing
// console.log(Number.parseInt('30px'));
// console.log(Number.parseInt('es23')); //NaN
// console.log(Number.parseInt('  2.5rem  '));
// console.log(Number.parseFloat(' 2.5rem  '));

// // Binary, Hex
// console.log(Number.parseInt('0x100', 16));
...
// console.log(isEvenNumber(1024));

// // Set every even movements row to orange if labelBalance is clicked
...
// const PI = 3.14159;
// console.log(PI);
// // const mathPI = +Math.PI.toFixed(5);
...
// console.log(new Date('Feb 10 2024 00:04:59'));
...
// console.log(future.toLocaleDateString().replaceAll('/', '.'));

// console.log(future.getTime()); //2142249810000
// console.log(new Date(2142249810000));
// console.log(Date.now()); //ms timestamp
...
// const dayToMs = 24 * 60 * 60 * 1000;
// const calulateDaysPassed = (date1, date2) => Math.abs(date2 - date1) / dayToMs;

// const days1 = calulateDaysPassed(new Date(2037, 3, 14), new Date(2037, 3, 4));
// console.log(days1);
